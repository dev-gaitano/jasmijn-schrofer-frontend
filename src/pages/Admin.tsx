...
const Admin = () => {
  ...
  const deleteImage = useCallback(async (imageId: string, publicId: string) => {
    try {
      ...
      toast.info('Note: Image still exists in Cloudinary. Use backend API to delete completely.');
    } catch (error) {
          ...
          <div className="min-h-screen bg-background p-6">
            <div className="max-w-7xl mx-auto">
              ...
              <Card className="mb-8 border-warning/20 bg-warning/5">
                <CardHeader>
                  <CardTitle className="text-warning">⚠️ Setup Required</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <p className="text-sm">To enable Cloudinary uploads, you need to:</p>
                  <ol className="list-decimal list-inside space-y-1 text-sm text-foreground-muted">
                    <li>Create a Cloudinary account at <a href="https://cloudinary.com" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">cloudinary.com</a></li>
                    <li>Set up an upload preset in your Cloudinary dashboard</li>
                    <li>Replace "your_cloud_name" and "your_upload_preset" in the code with your actual values</li>
                    <li>Update the admin email check in the code or set role="admin" in Clerk user metadata</li>
                  </ol>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <Button onClick={fetchImages} disabled={loading} variant="outline">
                ...
                <CardContent>
                  {images.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      {images.map((image) => (
                        <div key={image.id} className="bg-background-muted rounded-lg overflow-hidden border">
                          ...
                          <div className="p-4 space-y-3">
                            <div>
                              ...
                              <p className="text-xs text-foreground-muted">
                                {(image.bytes / 1024 / 1024).toFixed(2)} MB • {image.format.toUpperCase()}
                              </p>
